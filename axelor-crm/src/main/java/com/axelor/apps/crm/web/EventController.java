...
public class EventController {
		...
		try {
			...
			if (event.getLocation() != null) {
				...
				if (result != null) {
					...
					response.setFlash(
							String.format(I18n.get(com.axelor.apps.base.exceptions.IExceptionMessage.ADDRESS_5),
									event.getLocation()));
			} else
	...
	public void generateRecurrentEvents(ActionRequest request, ActionResponse response) throws AxelorException {
		try {
			...
			if (eventId == null)
				throw new AxelorException(Event.class, TraceBackRepository.CATEGORY_INCONSISTENCY,
	...
	/**
	 * This method is used to add attendees/guests from partner or contact partner
	 * or lead
	 *
	 * @param request
	 * @param response
	...
	public void computeTotalCount(ActionRequest request, ActionResponse response) {

		response.setValue("$records", eventService.getTotalCount());

	}

	public void showEvents(ActionRequest request, ActionResponse response) {
		String ids = eventService.getEvents((int) (long) request.getContext().get("id"));
		if (ids != null) {
			response.setView(ActionView.define("Event").model(Event.class.getName()).add("grid", "event-grid")
					.add("form", "event-form").domain("self.id in " + ids).map());
		} else {
			response.setFlash("No Records to Display");
		}
	}

	public void removeFromEvents(ActionRequest request, ActionResponse response) {
		String eventIds = eventService.getEvents((int) (long) request.getContext().get("id"));
		if (eventIds != null) {
			if (eventService.removeRelatedFromEvents(eventIds)) {
				response.setFlash("Records Updated Successfully");
				response.setReload(true);
			} else {
				response.setFlash("Unable to Update Records");
			}
		} else {
			response.setFlash("No Records to Update");
		}
