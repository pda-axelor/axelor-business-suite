...
public class EventServiceImpl implements EventService {
	...
	@Inject
	public EventServiceImpl(EventAttendeeService eventAttendeeService, PartnerService partnerService,
			EventRepository eventRepository, MailFollowerRepository mailFollowerRepo, ICalendarService iCalendarService,
	...
	@Override
	public Event createEvent(LocalDateTime fromDateTime, LocalDateTime toDateTime, User user, String description,
			int type, String subject) {
		...
		if (attendeesSet != null) {
			for (ICalendarUser user : attendeesSet) {
				if (user.getUser() != null) {
					...
					MailAddress mailAddress = Beans.get(MailAddressRepository.class).findOrCreate(user.getEmail(),
							user.getName());
	...
	@Override
	@Transactional
	public void addRecurrentEventsByDays(Event event, int periodicity, int endType, int repetitionsNumber,
			LocalDate endDate) {
	...
	public void addRecurrentEventsByWeeks(Event event, int periodicity, int endType, int repetitionsNumber,
		...
		List<DayOfWeek> dayOfWeekList = daysCheckedMap.keySet().stream().sorted().map(DayOfWeek::of)
	...
	public void addRecurrentEventsByMonths(Event event, int periodicity, int endType, int repetitionsNumber,
		...
		if (monthRepeatType == RecurrenceConfigurationRepository.REPEAT_TYPE_MONTH) {
			nextStartDateTimeFunc = dateTime -> dateTime.withDayOfMonth(1).plusMonths(periodicity)
			...
			nextStartDateTimeFunc = dateTime -> {
				...
				dateTime = baseNextDateTime
						.with(TemporalAdjusters.dayOfWeekInMonth(weekNo, event.getStartDateTime().getDayOfWeek()));

				if (!dateTime.getMonth().equals(baseNextDateTime.getMonth()) && weekNo > 1) {
					dateTime = baseNextDateTime.with(
							TemporalAdjusters.dayOfWeekInMonth(weekNo - 1, event.getStartDateTime().getDayOfWeek()));
				}
	...
	@Override
	@Transactional
	public void addRecurrentEventsByYears(Event event, int periodicity, int endType, int repetitionsNumber,
			LocalDate endDate) {
		Event lastEvent = event;
		if (endType == RecurrenceConfigurationRepository.END_TYPE_REPET) {
			int repeated = 0;
	...
	public String computeRecurrenceName(RecurrenceConfiguration recurrConf) {
		...
		case RecurrenceConfigurationRepository.TYPE_WEEK:
			...
			if (recurrConf.getMonday() && recurrConf.getTuesday() && recurrConf.getWednesday()
					&& recurrConf.getThursday() && recurrConf.getFriday() && !recurrConf.getSaturday()
					&& !recurrConf.getSunday()) {
				recurrName += I18n.get("every week's day");
			} else if (recurrConf.getMonday() && recurrConf.getTuesday() && recurrConf.getWednesday()
					&& recurrConf.getThursday() && recurrConf.getFriday() && recurrConf.getSaturday()
					&& recurrConf.getSunday()) {
		...
		case RecurrenceConfigurationRepository.TYPE_MONTH:
			if (recurrConf.getPeriodicity() == 1) {
				...
				recurrName += String.format(I18n.get("Every %d months the %d"), recurrConf.getPeriodicity(),
		...
		case RecurrenceConfigurationRepository.TYPE_YEAR:
			if (recurrConf.getPeriodicity() == 1) {
				...
				recurrName += String.format(I18n.get("Every %d years the %s"), recurrConf.getPeriodicity(),
	...
	public EmailAddress getEmailAddress(Event event) {
		...
		} else if (event.getPartner() == null && event.getContactPartner() == null && event.getLead() != null) {
	...
	public List<Long> getModelIds(String relatedModel, List<Long> list) {
		String modelName = this.getModelName(relatedModel);
		String statement = String.format("select p.id from %s p where p.id in :ids", modelName);
	...
	public String getModelName(String relatedModel) {
		String[] words = relatedModel.split("\\.");
		return words[words.length - 1];
	}

	@SuppressWarnings("unchecked")
	public List<Long> getRelatedIds(String relatedModel) {
		Query query = JPA.em()
				.createQuery("select e.relatedToSelectId from Event e where e.relatedToSelect = :relatedModel");
		query.setParameter("relatedModel", relatedModel);
		return query.getResultList();
	...
	public String getEvents(int selectId) {
		List<String> relatedModels = this.getSelectionOptions("crm.event.related.to.select");
		String model = relatedModels.get(Math.abs(selectId) - 1);
		List<Long> list1 = this.getRelatedIds(model);
		if (list1.isEmpty())
			return null;
		List<Long> list2 = this.getModelIds(model, list1);
		list1.removeAll(list2);
		Query query = JPA.em().createQuery(
				"select e.id from Event e where e.relatedToSelect = :model AND e.relatedToSelectId in :ids");
		query.setParameter("model", model);
		query.setParameter("ids", list1);
		List<Long> eventIds = query.getResultList();
		return eventIds.toString().replace('[', '(').replace(']', ')');
	}

	@Override
	@Transactional
	public boolean removeRelatedFromEvents(String eventIds) {
		List<Event> eventList = Beans.get(EventRepository.class).all().filter("self.id in " + eventIds).fetch();
		for (Event l : eventList) {
			l.setRelatedToSelect(null);
			l.setRelatedToSelectId(null);
			eventRepo.save(l);
		}
		return true;
	}
